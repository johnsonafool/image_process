...
from util import (
    ...
    get_rotated_mouth_loc_with_height,
    mouth_aspect_ratio,
)
...
while True:
    ...
    if checkKey(result, "error"):
        ...
        cv2.putText(
            frame, message, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1
        )
        continue
    else:
        ...
        # Cut upper lip part
        roi = image_ret[pts[1][1] : pts[0][1], pts[1][0] : pts[2][0]]
        x1, y1, c1 = roi.shape
        ...
        cv2.putText(
            frame,
            "tongue tip location: " + str(x_fin) + "," + str(y_fin),
            (10, 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 0, 0),
        ...
        if mouthMAR > MOUTH_AR_THRESH and y_fin > 1:
            color = (0, 0, 255)
            image = cv2.circle(
                frame[pts[1][1] : pts[0][1], pts[1][0] : pts[2][0]],
                (x_fin, y_fin),
                4,
                color,
                thickness=-1,
